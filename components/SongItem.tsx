...
const SongItem: React.FC<SongItemProps> = ({ data, onClick }) => {
    ...
    return (
        <div onClick={() => onClick(data.id)} className="relative group flex flex-col items-center justify-center rounded-md overflow-hidden gap-x-4 bg-neutral-400/5 cursor-pointer hover:bg-neutral-400/10 transition p-3">
            <div className="relative aspect-square rounded-md w-full h-full overflow-hidden">
                <Image className="object-cover" src={imagePath || "./assets/liked.png"} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" alt="Song-Image-Cover"/>
            </div>
