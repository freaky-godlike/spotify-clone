...
const createOrRetrieveCustomer = async ({ email, uuid }: { email: string, uuid: string }) => {
    ...
    if (error || !data?.stripe_customer_id) {
        const customerData: { metadata: { supabaseUUID: string }; email?: string } = { metadata: { supabaseUUID: uuid } };
...
const copyBillingDetailsToCustomers = async (uuid: string, payment_method: Stripe.PaymentMethod) => {
...
const manageSubscriptionStatusChange = async (subscriptionId: string, customerId: string, createAction = false) => {
    ...
    if (createAction && subscription.default_payment_method && uuid) 
        await copyBillingDetailsToCustomers(uuid, subscription.default_payment_method as Stripe.PaymentMethod);
};
